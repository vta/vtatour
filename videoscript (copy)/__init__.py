        ...
        while i<(file_count-2):
            ...
            try:
                if flag==1 or float(val[0])<14000:
                    os.system('ffmpeg -y -i '+base_dir+pdir+dirname+'/original/forward/'+str(i)+'.jpg'+' -vf "crop=in_w:in_h-50" -loglevel quiet '+base_dir+pdir+dirname+'/cropped/forward/'+str(n)+'_cropped.jpg')
...
def postgreSQL(rId,dId):
    ...
    for k in range(0,len(geopts)):
        try:
            ang=calculate_initial_compass_bearing((geopts[k]['lat'],geopts[k]['lng']),(geopts[k+1]['lat'],geopts[k+1]['lng']))
            sdata.append(geocords(1,ang,geopts[k]['lat'],geopts[k]['lng'],geopts[k+1]['lat'],geopts[k+1]['lng']))
        except:
                    ...
                    for i in range(0,len(sdata)):
                        if(rmframes(sdata[i])==1):
                            try:
                                ...
                                kml='forward'
                                grabimage(sdata[i],ang+0,'400x400',base_dir,pdir,dirname,'forward',m)
